...
template <typename T> constexpr int pct(T x) { return sizeof(T) <= 4 ? __builtin_popcount(x) : __builtin_popcountll(x); }
 
// Type: int (0-indexed position of Highest Set Bit MSB, returns -1 if x == 0)
template <typename T> constexpr int topbit(T x) { return x == 0 ? -1 : (sizeof(T) <= 4 ? 31 - __builtin_clz(x) : 63 - __builtin_clzll(x)); }
 
// Type: int (0-indexed position of Lowest Set Bit LSB, returns -1 if x == 0)
template <typename T> constexpr int lowbit(T x) { return x == 0 ? -1 : (sizeof(T) <= 4 ? __builtin_ctz(x) : __builtin_ctzll(x)); }
 
...
 
bool isprime(ll a){if (a<2) return 0; for (int i=2;i*i<=a;i++) if (a%i==0) return 0; return 1;}
 
ll cdiv(ll a, ll b) {
...
if (s%2==1){
   ...
   {
       j = n-1;
       while(j>=0 && (a[j]>=0 || a[j]%2==0)){
           j--;
       }
        l = 0;
       while (l < n - 1 && !(a[l] > 0 && a[l] % 2 == 1)){
           l++;
 
       }
       if(a[l]%2==1){
 
        cout << max(s + a[j], s - a[l]) << endl;
   }
else{
    cout << s + a[j] << endl;
 

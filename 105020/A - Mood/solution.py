...
 
// 2. Binary to Decimal using a loop
// Takes an integer whose digits visually represent binary (e.g., 1101) and returns a decimal int
...
set<pair<ll, ll>> ka, qa;
for(auto[dx,dy]:m){
   ka.insert({xk+dx,yk+dy});
   qa.insert({xq+dx,yq+dy});
}
for( auto pos:ka){
   if(qa.count(pos)){
      s++;

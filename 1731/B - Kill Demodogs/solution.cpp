...
#define print_container(arr) for(auto const temp: arr){ cout << temp << " ";  }cout <<endl;
...
vector<int> get_factors(int x) { // O(log X), needs sieve() first
    vector<int> factors;
    while (x > 1) { factors.push_back(spf[x]); x /= spf[x]; }
    return factors;
}
...
void solve(){
    ...
    m += n%mod * (n + 1)%mod * (2 * n + 1)%mod*inv6%mod;
    m += n %mod* (n - 1)%mod * (n + 1)%mod*inv3%mod;
   cout<< m%mod * 2022 % mod << endl;
}
#ifndef ONLINE_JUDGE
